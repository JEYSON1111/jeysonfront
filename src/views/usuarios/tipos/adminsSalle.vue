<template>
    <vx-card>
        <div>
            <p class="text-primary text-2xl font-medium">Lista de usuarios Salle administradores</p>
            <vs-divider></vs-divider>
            <vs-table multiple stripe v-model="codigoSelected"  pagination max-items="25" search :data="arregloAdmins">
                <template slot="header">
                    <div style="display:flex;">
                        <vs-chip color="success" style="font-size:12px;">
                          Total: {{arregloAdmins.length}}
                        </vs-chip>
...
export default{
    ...
    methods:{
        ...
        getUsuarios(){
            let me = this;
            me.$vs.loading();
            this.$http.get(this.$server_url+'salleadministrador')
                .then(function (res) {
                    me.$vs.loading.close();
                    me.arregloAdmins = res.data
                })
        ...
        //metodo par descargar el excel
           exportToExcel() {
            let headerTitle = ['Administradores salle' ,'Cedula', 'Grupo'];
