    ...
    <div>
        ...
        <vs-row vs-justify="center">
            <vs-col type="flex" vs-justify="center" vs-align="center" vs-w="12">
                <vs-card>
                    <div slot="header">
                        <p style="color:#0F93E4;font-weight: 400;">
                            <i class="fa fa-file" aria-hidden="true"></i> Reporte de Liquidación de codigos de barras
                        </p>
                    </div>
                    <div>
                        <div>
                        ...
                        <br>
                        <vs-button color="primary" size="small" type="relief" @click="filtrarLiquidacion();">
                            Filtrar
                        </vs-button>
        ...
        <div>
            <vs-table multiple v-model="codigoSelected" pagination max-items="25" search :data="arregloLiquidacion">
                <template slot="header" v-if="reportes  == true">
                    <p>Total: {{ sumaTotal }}</p>
                    <vs-button color="success" style="margin-left:20px;" size="small" @click="reporte()" type="border" icon="folder"></vs-button>
                    <vs-button color="warning"  size="small" class="mr-5 ml-3" style="font-size: 14px;" @click="activePrompt=true;">Exportar</vs-button>
                </template>
                <template slot="thead">
    ...
    methods:{
      ...
      //para limipiar los campos del excel
        ...
        //metodo par descargar el excel
           exportToExcel() {
            let headerTitle = ['serie' ,'libro', 'cantidad', 'asesor','Institucion'];
            let headerVal = [ 'serie','nombrelibro','cantidad',  'asesor','nombreInstitucion'];
       ...
       //lista de ciudades para usar en el select
        listaCiudades() {
         ...
         filtrarLiquidacion(){
                ...
                me.$vs.loading()
                const formData = new FormData();
                  formData.append('periodo_id',me.liquidacion.periodo.idperiodoescolar );
                    formData.append('institucion_id',me.liquidacion.institucion.idInstitucion );


                this.$http.post(this.$server_url+'bliquidacionSistema', formData).then(res => {
